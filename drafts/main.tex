...
without breaking the frozen backbone ($\sim$54\% SR untrained on
LIBERO-Spatial, $-$26\,pp from the OpenVLA-FT ceiling),
...
\begin{enumerate}
  ...
  \item \textbf{Empirical study of PSE-prefix injection.} We show that
        ...
        position preserves $\sim$54\% SR on LIBERO-Spatial
        (un-trained, $-$26\,pp vs.\ no-prefix), and that naive
...
and evaluate without any supervised fine-tuning. This isolates whether
...
$n=10$ rollouts/task, $n=100$ total. Baseline column reproduces
\texttt{openvla-7b-finetuned-libero-spatial} without PSE.}
\label{tab:pse_prefix_untrained}
\small
\begin{tabular}{lcccccccccccc}
\toprule
Config & T0 & T1 & T2 & T3 & T4 & T5 & T6 & T7 & T8 & T9 & Mean & vs.\ base \\
\midrule
OpenVLA-FT (no PSE) & 86 & 82 & 90 & 84 & 68 & 94 & 92 & 74 & 78 & 54 & 80.2 & --- \\
+ PSE prefix (untrained) & 70 & 80 & 30 & 70 & 40 & 10 & 80 & 100 & 10 & \todo{T9} & $\sim$54 & $-$26 \\
\bottomrule
\end{tabular}
\end{table}
...
remain at or above 80\%). The prefix is therefore \emph{not freely
benign}: it occupies eight position slots between image and text,
shifting downstream RoPE offsets and disrupting cross-modal attention
patterns the backbone learned during finetuning. The remaining $54\%$
SR floor, however, demonstrates that the backbone retains useful
behavior under modest position-embedding perturbation; this is the
prerequisite for the PSE prefix to ever \emph{help} once trained.
...
initialization (random vs.\ zero-init final projection), and ablate the
gripper-channel preprocessing. Five distinct training configurations
(autonomy\_v2c-* runs) all degrade the SR below the untrained baseline of
Section~\ref{sec:exp_pse_prefix}. The best of the trained runs
(\texttt{autonomy\_v2c-20260512-071037}; lr$=2\!\times\!10^{-4}$, default
PSE init) reaches a mean SR of $22\%$ on LIBERO-Spatial, with per-task
range 0--50\%. Lower-lr variants plateau in CE loss without recovering
the baseline SR.
