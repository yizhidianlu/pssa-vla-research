...
without breaking the frozen backbone (54.4\% SR untrained over the first 9
of 10 LIBERO-Spatial tasks, $-$25.8\,pp from the OpenVLA-FT ceiling),
...
\begin{enumerate}
  ...
  \item \textbf{Empirical study of PSE-prefix injection.} We show that
        ...
        position preserves 54.4\% SR over the first 9 LIBERO-Spatial
        tasks (un-trained, $-$25.8\,pp vs.\ no-prefix), and that naive
...
+ PSE prefix (untrained) & 70 & 80 & 30 & 70 & 40 & 10 & 80 & 100 & 10 & --- & 54.4$^\dagger$ \\
\bottomrule
\end{tabular}
\\[2pt]
{\footnotesize $^\dagger$Mean across the 9 completed tasks at draft
submission; final 10-task aggregate to be updated once T9 result is
pulled from the persistent disk on the next machine boot.}
\end{table}
...
untrained baseline (54.4\%). The best trained run reaches a 100-rollout
...
\midrule
Untrained variant\_A (ceiling) & --- & random & --- & 54.4\%$^\dagger$ \\
\midrule
