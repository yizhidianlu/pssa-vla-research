...
\midrule
OpenVLA-native control (no wrapper) & --- & --- & --- & \textbf{<NATIVE_SR>}\% \\
...
at matched $n{=}10$/task. The native baseline reaches
<NATIVE_SR>\% (Table~\ref{tab:trained_configs} row 1), which
[<NATIVE_INTERP>: ``confirms our wrapper preserves baseline behavior and the regression is fully attributable to the PSE prefix'' \emph{if} this number $\geq 80$\%, or ``shows part of the regression is wrapper-induced and isolates [Z]\,pp as PSE-attributable'' otherwise]. To further isolate whether LoRA or the PSE encoder is the destructive
